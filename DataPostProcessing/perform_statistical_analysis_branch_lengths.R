...
#Figure out the expected length of each branch
p_2 <- 0.11
p_3 <- 0.094
p_4 <- 0.0384
p_5 <- 0.0576 
...
extractRequiredStatistics <- function(pop_dat, w_test_dat){
  ...
  percent_error_values_col <- abs((pop_dat[,1] - expected_means[1])/expected_means[1]) * 100
  ...
  for (i in 2:ncol(pop_dat)){
    percent_error_values_col <- abs((pop_dat[,i] - expected_means[i])/expected_means[i]) * 100
...
#Create rows to fomat data for an output table
...
plot1 <- ggplot(data = kimura_percent_error_values, aes(x=factor(Expected_Substitutions), y =  Error_0.001)) + geom_boxplot() +xlab("Expected Subsitutions") + ylab("Percent Error (%)")+ggtitle("A") + ylim(-110, 650)
plot2 <- ggplot(data = kimura_percent_error_values, aes(x=factor(Expected_Substitutions), y =  Error_0.05)) + geom_boxplot() +xlab("Expected Subsitutions") + ylab("Percent Error (%)")+ggtitle("B")+ ylim(-110, 650)
plot3 <- ggplot(data = kimura_percent_error_values, aes(x=factor(Expected_Substitutions), y =  Error_0.1)) + geom_boxplot() +xlab("Expected Subsitutions") + ylab("Percent Error (%)")+ggtitle("C")+ ylim(-110, 650)
plot4 <- ggplot(data = kimura_percent_error_values, aes(x=factor(Expected_Substitutions), y =  Error_0.2)) + geom_boxplot() +xlab("Expected Subsitutions") + ylab("Percent Error (%)")+ggtitle("D")+ ylim(-110,650)
...
abs_percent_error_values <- abs(not_kimura_percent_error_values)
colnames(abs_percent_error_values) <- c("0.001", "0.05", "0.1", "0.2")
abs_percent_error_values_kimura <- abs(kimura_percent_error_values)
colnames(abs_percent_error_values_kimura) <- c("0.001", "0.05", "0.1", "0.2")


boxplot(not_kimura_percent_error_values[,-1], ylab = "Absolute Percent Error (%)", xlab = "Population Mutation Rate Parameter (??)")
boxplot(kimura_percent_error_values[,-1], ylab = "Absolute Percent Error (%)", xlab = "Population Mutation Rate Parameter (??)")

boxplot(abs_percent_error_values[,-1], ylab = "Absolute Percent Error (%)", xlab = "Population Mutation Rate Parameter (??)")
boxplot(abs_percent_error_values_kimura[,-1], ylab = "Absolute Percent Error (%)", xlab = "Population Mutation Rate Parameter (??)")




#Compare error values in a statistically significant manner
wilcox.test(abs_percent_error_values$Error_0.001, abs_percent_error_values_kimura$Error_0.001, alternative = "less")
wilcox.test(abs_percent_error_values$Error_0.05, abs_percent_error_values_kimura$Error_0.05, alternative = "less")
wilcox.test(abs_percent_error_values$Error_0.1, abs_percent_error_values_kimura$Error_0.1, alternative = "less")
wilcox.test(abs_percent_error_values$Error_0.2, abs_percent_error_values_kimura$Error_0.2, alternative = "less")
